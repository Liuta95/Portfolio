...
- HealthConnect case study (`healthconnect.html`): real client/employer project, UX/UI Designer, Mar 2024 – Jan 2025. No public live link or repo (confidential). Key Screens images in `assets/healthconnect/` are exported directly from the production Figma file (via the Figma MCP, given a node URL per screen) with only the product's logo removed (canvas-painted out over the flat background it sits on) — content, layout, and color are otherwise unedited real design, not a recreation. Currently one screen (`doctor-home.png`); more are expected to be added the same way as the user supplies node URLs — restore the horizontal `.case-slider` markup (CSS already in place, unused while count is 1) once there are 2+. This is disclosed directly on the case-study page under the Key Screens heading — keep that disclosure line accurate to whatever sourcing is true at the time if the section is edited. An earlier version of this section used hand-built HTML/CSS recreations instead — superseded by this real-export approach.
